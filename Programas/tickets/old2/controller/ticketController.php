<?php 
	...
	class ticket extends globalController{
		...
		public function getTicket(){
			...
			$this->where = ($this->where == '') ? 'WHERE t.nEstado != 3' : $this->where;
		...
		private function setTicket($data){	//Creo un array a base de los registros del ticket
			while($row = $data->fetch()){
				...
				$equipo = ($row['idEquipo'] != NULL) ? $Equipo->getEquipo($row['idEquipo']) : array(array('Patrimony' => null, 'Intern' => null, 'TypeName' => null, 'Brand' => null, 'Model' => null));
				...
				$this->listTicket[] = array(
					...
					'Patrimony' => $equipo[0]['Patrimony'],
					'NroEquipo' => $equipo[0]['Intern'],
					'Equipo' => $equipo[0]['TypeName'],
					'Marca' => $equipo[0]['Brand'],
					'Modelo' => $equipo[0]['Model'],
...
//-------------------------------------------------- OBSOLETO POR EL MOMENTO ¡¡¡NO DESCOMENTAR!!!! -----------------------------------------------------
		...
		private function getIdEquipo(){
			if(!isset($_POST['ingreso'])){	// Remplazar '24' de ser necesario por el id correspondiente al 'ingreso de pc'
				return NULL;
			}
			...
			$idPat = ($existPat) ? $Equipo->getIdBy('PATRIMONIO', $patrimonio) : null;

			if($existIntern){
				if($Equipo->getIdBy('INTERNO', $interno) == $idPat){
			...
			// if($this->existingEquipo('PATRIMONIO', $patrimonio)){
			// 	return $Equipo->getIdBy('PATRIMONIO', $patrimonio);
			// }

			// return $Equipo->addEquipoFromTicket($this->getUserDependence($this->ticket['Client']), false);
			...
			switch ($call) {
				case 'insert':
					if(isset($_POST['motivo'], $_POST['ingreso'])){
						if(!$this->validateEmptyPost(array('obs','alta_compartida','marca_ingreso','modelo_ingreso','patrimonio_ingreso','ingreso','so_ingreso','ram_ingreso','tipo_disco_ingreso','cantidad_disco_ingreso'))){
							$valid = true;
						}
					}
				break;
