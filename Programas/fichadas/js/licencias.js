...
async function start(){
    $("#cerradoMotivoField").hide();

    $("#cerrado").change(function(e) {
        if (this.checked) 
            $("#cerradoMotivoField").show();
        else
            $("#cerradoMotivoField").hide();
    });
...
$('#grillaLicencias').on('click','.editLicencia',async function(){
    ...
    if (licencia[0].Cerrado == "1")
        $("#cerrado").prop("checked", true);

    $('#cerradoMotivo option[value=\''+licencia[0].CerradoMotivoId+'\']').prop('selected', true);

    console.log($('#cerradoMotivo option[value=\''+licencia[0].CerradoMotivoId+'\']'));
    console.log(licencia);

    $("#cerrado").change();
    $("#cerradoMotivo").change();
...
async function displayTable(modal = true){
    ...
    if(typeof filterCerrado != 'undefined') {
        if(filterCerrado != "TODOS") {
            let closed = filterCerrado == "CERRADO"
            if (closed)
                filterTable = filterTable.filter(t => t.Cerrado == closed);
            else
                filterTable = filterTable.filter(t => t.Cerrado == closed || t.Cerrado == null);
