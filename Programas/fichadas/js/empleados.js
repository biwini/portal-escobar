...
async function start(){
    ...
    listInactiveEmployee = await getInactiveEmployees();
    ...
    $("#buttonTabListInactiveEmployee").click(async function(){
        listInactiveEmployee = await getInactiveEmployees();
        DataTableInactive.columns.adjust().draw(false);
    });
    ...
    DataTableInactive = $('#tableInactiveEmployeesPaginated').DataTable({
        "language": {
            "url": "src/Datatables/Spanish.json"
        },
        "data": listInactiveEmployee,
        "deferRender":true,
        "scrollX":true,
        "scrollCollapse":true,
        "lengthMenu":[[10, 20, 25, 50, -1], [10, 20, 25, 50, "Todos"]],
        ...
        "columns":[
            ...
            { "data": "acciones",
                "render":function(data, type, full, meta){
                    return '<button title=\'Restaurar\' type=\'button\' class=\'btn btn-md btn-primary restore\' value=\''+full.id+'\' title=\"Restaurar\">Restaurar</button> ';
                }
            },
        ],
    });
    ...
    $("#tableInactiveEmployeesPaginated").on("click",".restore",function(){
        swal({
            title: "!Restaurando Empleado!",
            text: "¿Seguro que desea restaurar este empleado?",
            icon: "warning",
            buttons: {
            cancel: "No",
            Si: true,
                ...
                $.ajax({
                    ...
                    success:function(data){
                        if(data['status'] == 'error'){
                            ...
                            actualizarDatos();
                            swal('Se restauro el empleado','','success');
                        }
                    }
                });
            }
...
async function actualizarDatos(){
    ...
    DataTableInactive.rows().remove().draw();
    DataTableInactive.rows.add(listInactiveEmployee);
    DataTableInactive.columns.adjust().draw();
}
...
function getInactiveEmployees(action = 'getInactiveEmployeesPaginated', id = 0, page = 'Empleados'){
    return new Promise(resolve => { 
        let list;
