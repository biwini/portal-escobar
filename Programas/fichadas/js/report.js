        ...
        case 1:
            trh.html("<tr>\
                ...
                <tr>\
                    ...
                    <th class='hsExtrasTrabajadas'></th>\
                    <th colspan='6'></th>\
                </tr>");
            break;
        case 2:
        ...
        switch(type){
            ...
            case 2:
                trb.append("<tr class='"+dFrom.format("YYYY-MM-DD")+"' day-week='"+(dFrom.day())+"' is-holiday='"+hl+"' is-licencia='"+cl+"'>\
                    <td>"+dFrom.format("YYYY-MM-DD")+"</td>\
                    <td class='weekDay'>"+getIsoWeekday(dFrom.day())+"</td>\
                    <td class='t_in_out'></td>\
                    <td class='amount_in_out'></td>\
                    <td class='totalHours'></td>\
                </tr>");
                break;
        }
        ...
        switch(type){
            case 1:
                ...
                $.each(data,function(key,value){
                    report.find("."+key+" .reloj").html(value[0].reloj);

                    if(value.length == 1){
                        dayWeek = report.find("."+key).attr("day-week");
                        findDay=horarios.find(e => e.dni === dni && e.idDiaSemana === dayWeek);

                        if(findDay){
                            a=moment(key+" "+value[0].hora);
                            b=moment(key+" "+findDay.tEntrada.substr(0,5));
                            c=moment(key+" "+findDay.tSalida.substr(0,5));
                            if(a.diff(b,"m") > a.diff(c,"m")){
                                inOut=".t1_in";
                            }
                            else if(a.diff(b,"m") > a.diff(c,"m")){
                                inOut=".t1_out";
                            }
                            else{
                                inOut=".t1_in";
            ...
            case 2:
                $.each(data,function(key,value){
                    ...
                    tr.find('.reloj').html(value.reloj);
...
function reportDetallado(dni,hoursWork){
    ...
    let extraHoursDuration = moment.duration(0, 'minutes');
    ...
    report.find(".tableReport tbody tr").each(function(){
        ...
        if(findDay){
            ...
            if(hourIn || hourOut){
                ...
                extraHoursDuration.add(extraHours, 'minutes');

                if(totalMinutes >= 21){
                    hoursWork=moment(hourOutDB).diff(hourInDB,"h");
                    ...
                    if(calcHoursWork>=hoursWork){
                        ...
                        ht = (hoursWork*60)+roundExtraHours;  
                    }
                    else{
        ...
        if($(this).attr("is-licencia")){
            $(this).find(".weekDay").append("<b> (Con licencia)</b>");
    ...
    report.find(".hsExtrasTrabajadas").html(
      `${extraHoursDuration.days() * 24 + extraHoursDuration.hours()} :
       ${moment(extraHoursDuration.as('milliseconds')).format('mm')} hs
      `
    );
}
                ...
                if(totalMinutes >= 21){
                    ...
                    if(minutesLate > 0 && minutesEarly > 0){
                        ...
                        if(minutesLate > 0){
                            ...
                            m=moment(hourOutDB).diff(hourInDB,"m");
                            h=moment(hourOutDB).diff(hourInDB,"h");
                            roundH=h*60;
                            restoH=m%60;
                            if(restoH >= 21){
                                roundH = roundH + 30;
...
function reportHoras(dni,hoursWork){
    ...
    $.each(data,function(key,value){
        ...
        hourIn = min;
        hourOut = max;
        todayLicencia=checkLicencia(isLicencia,date);
        ...
        if(findDay){
            if(hourIn || hourOut){
                ...
                if(totalMinutes >= 21){
                    ...
                    // console.log("hours="+roundH);
                    // console.log("extra="+roundExtraHours);
