...
$('.menu li a').click(function(){
	if(!$(this).hasClass('active')){
		switch($(this).text()){
			case 'USUARIOS': displayPageContent('usuarios'); getUser(); break;
			case 'PROGRAMAS': displayPageContent('programas'); getProgram(); break;
            case 'LOCALIDADES': displayPageContent('localidades'); getLocation(); break;
			case 'DEPENDENCIAS': displayPageContent('dependencias'); getSecretary(); break;
            case 'SECRETARIAS': displayPageContent('secretarias'); getSecretary(); break;
			case 'ACCESOS': displayPageContent('accesos'); getAccess(); break;
			case 'AUDITORIAS': displayPageContent('auditorias'); AuditoriaDataTable.destroy(); displayAuditoriaTable(); break;
            case 'COMPARTIDAS': displayPageContent('compartidas'); CompartidaDataTable.destroy(); displayCompartidaTable(); break;
            case 'EQUIPOS': displayPageContent('equipos'); EquipoDataTable.destroy(); displayEquipoTable(); break;
		}
	}
});
    ...
    $.each(ListSecretaria, function(i,s){
        if(s.IdSecretaria == selected){
            $.each(s.Dependences, function(n,d){
                $('.dependencia').append('<option data-tokens=\''+d.Name+','+d.IdDependence+'\' value=\''+d.IdDependence+'\'>'+d.Name+'</option>');
            });
        }
    });
...
function emptyDependence(){
    $('.dependencia').selectpicker('destroy');
    $('.dependencia').children('option:not(:first)').remove();
    // $('.dependencia').html('<option data-tokens=\'\' value=\'0\'>SELECCIONE UNA DEPENDENCIA</option>');
}
function setLocation(){
    $('.localidad').html('<option value=\'0\'>Seleccione una localidad</option>');
    $.each(ListLocalidad, function(i,l){
        $('.localidad').append('<option value=\''+l.Id+'\'>'+l.Localidad+'</option>');
    });
}
...
function displayPageContent(content,table){
	$('.menu li a').each(function() {
	    if($(this).hasClass('active')){
	    	$(this).removeClass('active');
	    }
	    if($(this).text() == content.toUpperCase()){
	    	$(this).addClass('active');
	    }
	});
	$('.display-content').each(function() {
	    if($(this)[0].id == 'div_'+content){
...
//                 inp.value = this.getElementsByTagName("input")[0].value.split(' | ')[1].trim();
