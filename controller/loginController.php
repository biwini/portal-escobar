<?php 
    ...
    include realpath(__DIR__ ).'/ldapController.php';

	class login extends globalController{
		public $user = array();
		public $program = array();
		private $cantPermisos = array();
		private $pages = array();
		private $url;
		private $legajo;
		private $pass;
		...
		public function loginUser(){
			if(!$this->validateFields('login')){
				return array('Status' => 'Formulario incompleto');
			}

			// if(!$this->validCaptcha()){
			// 	return array('Status' => 'No se pudo validar el captcha');
			// }

			$Ldap = new ldap();

			//COMPRUEBO QUE SEA UN AUTOLOGIN, ESTA VARIABLE DE SESSION SE DECLARA EN 'UserController.php' DENTRO DE ESTE MISMO DIRECTORIO
			if(isset($_SESSION['AUTO_LOGIN']) && $_SESSION['AUTO_LOGIN'] && $_SESSION['TEMPORAL_PASSWORD'] != NULL && $_SESSION['LEGAJO'] != NULL){
				$this->legajo = $_SESSION['LEGAJO'];
				$this->pass = $_SESSION['TEMPORAL_PASSWORD'];

				$_SESSION['AUTO_LOGIN'] = false; //VACIO LAS VARIABLES ANTES DE ELIMINARLAS
				$_SESSION['TEMPORAL_PASSWORD'] = null; //VACIO LAS VARIABLES ANTES DE ELIMINARLAS
				// $_SESSION['UNREGISTRED'] = false; //VACIO LAS VARIABLES ANTES DE ELIMINARLAS

				UNSET($_SESSION['AUTO_LOGIN']); // ELIMINO LA VARIABLE UNA VEZ UTILIZADA
			}else{
				$this->legajo = $this->cleanString($_POST['usuario']); //Filtro el Usuario
				$this->pass = $_POST['contrasenia'];
			}

			if($Ldap->validateUser($this->legajo,$this->pass)){

				$this->ExistingUser = $this->existingLegajo($this->legajo);
				if(!$this->ExistingUser){
					$_SESSION['LOGUEADO'] = true;
					$_SESSION['LEGAJO'] = $this->legajo;
					$_SESSION['TEMPORAL_PASSWORD'] = $this->pass;
					// $_SESSION['UNREGISTRED'] = true;

					return array('Status' => 'Success');
				}

				if($this->ExistingUser){ //Verifico si el usuario existe.
					...
					if($result){	//Verifico si la consulta fue correcta
						...
						// if($this->pass == password_verify($this->pass, $this->user['HashPass'])){ //Compruebo que la contrasenia ingresada sea la correcta ¡¡¡OBSOLETO!!!
						...
						return array('Status' => 'Usuario o contraseña incorrectos');
					}
				}
			}else{
				return array('Status' => 'Usuario o contraseña incorrectos');
		...
		private function existingLegajo($Legajo){
			$this->query = 'SELECT COUNT(cNLegajo) FROM Usuario WHERE cNLegajo = :Legajo';
			$this->data = [':Legajo' => $Legajo];

			return ($this->searchRecords() > 0) ? true : false;
		}

		// Esta funcion la utilizo para validar las llamadas realizadas por el sistema
